...
// ATUALIZADO: Rota de Admin para CONFIRMAR pagamento Pix e enviar e-mail
app.patch('/api/admin/confirm-pix/:paymentId', adminAuth, async (req, res) => {
    ...
    try {
        ...
        // NOVO: Envia o e-mail de confirmação
        const templateData = {
            username: user.username,
...
// ATUALIZADO: Rota de Admin para APROVAR saque e enviar e-mail
app.patch('/api/admin/withdrawals/:id/approve', adminAuth, async (req, res) => {
    const { id } = req.params;

    try {
        const withdrawal = await WithdrawalRequest.findById(id).populate('userId', 'username email');
        ...
        // NOVO: Envia o e-mail de aprovação
        const templateData = {
...
// ATUALIZADO: Rota de Admin para REJEITAR saque e enviar e-mail
app.patch('/api/admin/withdrawals/:id/reject', adminAuth, async (req, res) => {
    ...
    try {
        ...
        // NOVO: Envia o e-mail de rejeição
        const templateData = {
            username: user.username,
            amount: withdrawal.amount,
            date: withdrawal.processedAt
